...
def test_shared_and_legacy_paths_remain_global() -> None:
    assert get_cli_history_path() == Path.home() / ".aeloon" / "history" / "cli_history"
    assert get_bridge_install_dir() == Path.home() / ".aeloon" / "bridge"
    assert get_legacy_sessions_dir() == Path.home() / ".aeloon" / "sessions"
