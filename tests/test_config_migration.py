...
def test_onboard_refresh_rewrites_legacy_config_template(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("aeloon.core.config.loader.get_config_path", lambda: config_path)
    monkeypatch.setattr(
        "aeloon.cli.flows.onboard.get_workspace_path", lambda _workspace=None: workspace
    )
...
def test_onboard_refresh_backfills_missing_channel_fields(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "aeloon.channels.registry.discover_all",
        lambda: {
            "qq": SimpleNamespace(
...
def test_load_config_migrates_legacy_memory_backend_shape(tmp_path) -> None:
    config_path = tmp_path / "config.json"
    config_path.write_text(
        json.dumps(
            {
